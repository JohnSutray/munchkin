...
export class BattleBoardComponent {
  ...
  readonly brokenDoorId$ = merge(
    ...
    this.gameIterationService.updatesOfType$(GameActions.setBrokenDoorAction).pipe(map(this.getCurrentBrokenDoor)),
  );

  readonly inBattleState$ = merge(
    this.gameIterationService.updatesOfType$(GameActions.startBattleAction).pipe(mapTo(true)),
